...
function suggestIntent(relPath: string, filename: string): string {
  const lower = filename.toLowerCase().replace(/\.(md|txt|rst|adoc)$/, "");
  if (INTENT_MAP[lower]) return INTENT_MAP[lower];
  if (INTENT_MAP[filename.toLowerCase()]) return INTENT_MAP[filename.toLowerCase()];

  // Path-based hints
  const parts = relPath.toLowerCase().split("/");
  if (parts.some((p) => p === "adr" || p === "decisions")) return "Why was this architectural decision made?";
  if (parts.some((p) => p === "api" || p === "openapi")) return "What are the available API endpoints?";
  if (parts.some((p) => p === "guides" || p === "tutorials")) return "How do I use this feature?";
  if (parts.some((p) => p === "runbooks" || p === "ops")) return "How do I operate this service?";
  if (parts.some((p) => p === "security")) return "What are the security requirements or policies?";
...
function suggestId(relPath: string): string {
  return relPath
...
function scanDirectory(dir: string, maxDepth = 3, depth = 0): DiscoveredFile[] {
  ...
  for (const name of entries) {
    ...
    if (stat.isDirectory()) {
      ...
      if (depth === 0 && !CANDIDATE_DIRS.includes(lower)) continue;
      ...
      if (isRoot && !CANDIDATE_FILENAMES.has(lower)) continue;
...
export async function runInit(outputPath: string, yes = false): Promise<void> {
  ...
  let selectedFiles: DiscoveredFile[] = [];
  ...
  if (discovered.length === 0) {
    ...
    process.stdout.write("\n");
    const includeInput = await prompt(
      rl,
      `Include all ${discovered.length} file(s)? ${dim("(Y/n, or comma-separated numbers to exclude)")}: `,
    );
    ...
    if (includeVal === "" || includeVal === "y" || includeVal === "yes") {
      ...
      const excluded = new Set(
        includeVal.split(",").map((s) => parseInt(s.trim(), 10) - 1),
      );

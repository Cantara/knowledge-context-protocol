...
exports.HASH_ALGORITHMS = void 0;
 ...
 */
function computeContentDigest(target, algorithm) {
    try {
        const hashFile = (p) => (0, node_crypto_1.createHash)(algorithm).update((0, node_fs_1.readFileSync)(p)).digest("hex");
        ...
        const digest = (0, node_crypto_1.createHash)(algorithm);
        for (const e of entries)
            digest.update(`${e}\0${hashFile((0, node_path_1.join)(target, e))}\n`);
 ...
 */
function supersededCycleIds(successor) {
    const cycle = new Set();
    const state = new Map(); // 0/undefined = unvisited, 1 = in-path, 2 = done
    for (const start of successor.keys()) {
        ...
        while (node !== undefined && successor.has(node) && state.get(node) !== 2) {
            if (state.get(node) === 1) {
                for (const id of path.slice(path.indexOf(node)))
                    cycle.add(id);
                break;
            }
            state.set(node, 1);
...
function validate(manifest, manifestDir) {
    const errors = [];
    const warnings = [];
    // Root fields
    if (!manifest.project)
        errors.push("Root field 'project' is required");
    if (!manifest.version)
        warnings.push("manifest: 'version' not declared; RECOMMENDED per §6.2");
    if (manifest.units.length === 0)
        warnings.push("Manifest has no units");
    ...
    if (!manifest.kcp_version) {
        ...
        warnings.push(`manifest: unknown kcp_version '${manifest.kcp_version}'; processing as 0.8`);
    }
    ...
    for (const unit of manifest.units) {
        ...
        if (!unit.path)
            errors.push(`${ctx}: missing required field 'path'`);
        if (!unit.intent)
            errors.push(`${ctx}: missing required field 'intent'`);
        if (!unit.scope)
            errors.push(`${ctx}: missing required field 'scope'`);
        ...
        if (unit.scope && !VALID_SCOPES.has(unit.scope)) {
            errors.push(`${ctx}: 'scope' must be one of [global, module, project], got '${unit.scope}'`);
        }
        ...
        if (unit.access && !VALID_ACCESS_VALUES.has(unit.access)) {
            warnings.push(`${ctx}: unknown 'access' value '${unit.access}'; treating as 'restricted'`);
        }
        // auth_scope validation (§4.11)
        if (unit.auth_scope && unit.access !== "restricted") {
            warnings.push(`${ctx}: 'auth_scope' is only meaningful when access is 'restricted'`);
        }
        ...
        if (unit.delegation) {
            ...
            if (hitl !== undefined) {
                ...
                if (mech !== undefined && !["oauth_consent", "uma", "custom"].includes(mech)) {
                    errors.push(`${ctx}: delegation.human_in_the_loop.approval_mechanism must be one of [oauth_consent, uma, custom], got '${mech}'`);
                }
            }
            if (manifest.delegation?.max_depth != null &&
                unit.delegation.max_depth != null &&
                unit.delegation.max_depth > manifest.delegation.max_depth) {
                errors.push(`${ctx}: unit delegation.max_depth (${unit.delegation.max_depth}) must not exceed root delegation.max_depth (${manifest.delegation.max_depth})`);
            }
        }
        // compliance validation (§3.5)
        if (unit.compliance?.sensitivity) {
            if (!VALID_SENSITIVITY_VALUES.has(unit.compliance.sensitivity)) {
                errors.push(`${ctx}: compliance.sensitivity must be one of [confidential, internal, public, restricted], got '${unit.compliance.sensitivity}'`);
            }
        }
        ...
        if (unit.hints) {
            ...
            if (h.summary_available === true && !h.summary_unit) {
                warnings.push(`${ctx}: summary_available is true but no summary_unit declared`);
            }
            if (typeof h.summary_unit === "string" && !unitIds.has(h.summary_unit)) {
                warnings.push(`${ctx}: summary_unit references non-existent unit '${h.summary_unit}'`);
            }
            if (typeof h.chunk_of === "string" && !unitIds.has(h.chunk_of)) {
                warnings.push(`${ctx}: chunk_of references non-existent unit '${h.chunk_of}'`);
            }
            if (h.chunk_index != null && !h.chunk_of) {
                warnings.push(`${ctx}: chunk_index is present without chunk_of`);
            }
        }
            ...
            for (const [action, value] of Object.entries(unit.authority)) {
                if (value !== undefined && !VALID_AUTHORITY_VALUES.has(value)) {
                    if (KNOWN_AUTHORITY_ACTIONS.has(action)) {
                        warnings.push(`${ctx}: authority.${action} has unknown value '${value}'; expected initiative, requires_approval, or denied`);
                    }
                    else {
                        warnings.push(`${ctx}: authority custom action '${action}' has unknown value '${value}'; expected initiative, requires_approval, or denied`);
                    }
                }
            }
        }
        ...
        if (unit.discovery) {
            ...
            if (disc.verification_status === "rumored" &&
                disc.confidence !== undefined &&
                disc.confidence >= 0.5) {
                warnings.push(`${ctx}: discovery.verification_status is 'rumored' but confidence is ${disc.confidence} (>=0.5); consider upgrading status to 'observed'`);
            }
            if (disc.verification_status === "declared" &&
                disc.confidence !== undefined &&
                (disc.confidence < 0.5 || disc.confidence >= 0.8)) {
                warnings.push(`${ctx}: discovery.verification_status is 'declared' but confidence is ${disc.confidence}; SHOULD be in [0.5, 0.8) per RFC-0018 §5.1`);
            }
            if (disc.verified_at !== undefined &&
                (disc.verification_status === "rumored" ||
                    disc.verification_status === "declared" ||
                    disc.verification_status === "observed")) {
                warnings.push(`${ctx}: discovery.verified_at is set but verification_status is '${disc.verification_status}'; verified_at implies status should be 'verified'`);
            }
            if (disc.contradicted_by !== undefined && !unitIds.has(disc.contradicted_by)) {
                warnings.push(`${ctx}: discovery.contradicted_by references unknown unit id '${disc.contradicted_by}'`);
            }
        }
        // not_for validation (RFC-0015, v0.17)
        if (unit.not_for_strict !== undefined && (!unit.not_for || unit.not_for.length === 0)) {
            warnings.push(`${ctx}: 'not_for_strict' is set but 'not_for' is empty or absent`);
        }
        ...
        if (unit.content_structure) {
            ...
            if (cs.primary !== undefined && !VALID_CONTENT_MODALITIES.has(cs.primary)) {
                warnings.push(`${ctx}: content_structure.primary has unknown value '${cs.primary}'; expected one of prose, table, code, list, diagram, reference, mixed`);
            }
            if (cs.contains) {
                for (const modality of cs.contains) {
                    if (!VALID_CONTENT_MODALITIES.has(modality)) {
                        warnings.push(`${ctx}: content_structure.contains has unknown value '${modality}'; expected one of prose, table, code, list, diagram, reference, mixed`);
                    }
                }
            }
            if (cs.density !== undefined && !VALID_DENSITY.has(cs.density)) {
                warnings.push(`${ctx}: content_structure.density has unknown value '${cs.density}'; expected one of sparse, normal, dense`);
            }
        }
        ...
        if (unit.content_hash) {
            ...
            if (!ch.algorithm || !exports.HASH_ALGORITHMS.includes(ch.algorithm)) {
                errors.push(`${ctx}: content_hash.algorithm must be one of ${exports.HASH_ALGORITHMS.join(", ")}`);
            }
                ...
                const resolved = (0, node_path_1.resolve)((0, node_path_1.join)(manifestDir, unit.path));
                if (resolved.startsWith((0, node_path_1.resolve)(manifestDir)) && (0, node_fs_1.existsSync)(resolved)) {
                    const observed = computeContentDigest(resolved, ch.algorithm);
                    if (observed !== ch.value.toLowerCase()) {
                        errors.push(`${ctx}: content_hash does not match content on disk ` +
        ...
        if (manifestDir && unit.path) {
            const resolved = (0, node_path_1.resolve)((0, node_path_1.join)(manifestDir, unit.path));
            if (!resolved.startsWith((0, node_path_1.resolve)(manifestDir))) {
                errors.push(`${ctx}: path traversal rejected: '${unit.path}'`);
            }
            else if (!(0, node_fs_1.existsSync)(resolved)) {
    ...
    for (const rel of manifest.relationships) {
        if (!unitIds.has(rel.from_id)) {
            warnings.push(`Relationship references unknown unit id '${rel.from_id}'`);
        }
    ...
    for (const unit of manifest.units) {
        for (const dep of unit.depends_on) {
            if (!unitIds.has(dep)) {
                warnings.push(`Unit '${unit.id}': depends_on references unknown unit '${dep}'`);
            }
        }
    }
    ...
    const unitSuccessor = new Map();
    for (const unit of manifest.units) {
        const t = effectiveTemporal(unit.temporal);
        if (t.valid_from && t.valid_until && t.valid_until < t.valid_from) {
            warnings.push(`Unit '${unit.id}': temporal.valid_until '${t.valid_until}' precedes valid_from '${t.valid_from}' (empty validity window — the unit can never be active)`);
        }
        if (t.valid_until && t.valid_until < today && !t.superseded_by) {
            warnings.push(`Unit '${unit.id}': temporal.valid_until '${t.valid_until}' is in the past and no superseded_by is set (stale unit with no successor)`);
        }
        ...
        if (t.superseded_by && !t.superseded_by.includes(":")) {
            if (!unitIds.has(t.superseded_by)) {
                warnings.push(`Unit '${unit.id}': temporal.superseded_by references unknown unit '${t.superseded_by}'`);
            }
            else {
        ...
        if (disc?.verification_status === "verified" && !disc.verified_by) {
            warnings.push(`Unit '${unit.id}': discovery.verification_status is 'verified' but discovery.verified_by is absent`);
        }
    }
    for (const id of supersededCycleIds(unitSuccessor)) {
        errors.push(`temporal.superseded_by cycle detected involving unit '${id}'`);
    }
    if (manifest.discovery?.verification_status === "verified" &&
        !manifest.discovery.verified_by) {
        warnings.push("manifest: discovery.verification_status is 'verified' but discovery.verified_by is absent");
    }
    // Federation: manifests[].temporal (§3.6, RFC-0021).
    const refIds = new Set(manifest.manifests.map((m) => m.id));
    const refSuccessor = new Map();
    for (const ref of manifest.manifests) {
        const t = ref.temporal;
        if (!t)
            continue;
        if (t.valid_from && t.valid_until && t.valid_until < t.valid_from) {
            warnings.push(`manifests['${ref.id}']: temporal.valid_until '${t.valid_until}' precedes valid_from '${t.valid_from}' (empty validity window)`);
        }
        if (t.valid_until && t.valid_until < today && !t.superseded_by) {
            warnings.push(`manifests['${ref.id}']: temporal.valid_until '${t.valid_until}' is in the past and no superseded_by is set (stale federation link)`);
        }
        if (t.superseded_by) {
            if (!refIds.has(t.superseded_by)) {
                warnings.push(`manifests['${ref.id}']: temporal.superseded_by references unknown manifests[].id '${t.superseded_by}'`);
            }
            else {
    ...
    if (manifest.delegation?.human_in_the_loop !== undefined) {
        ...
        const mech = hitl.approval_mechanism;
        if (mech !== undefined && !["oauth_consent", "uma", "custom"].includes(mech)) {
            errors.push(`manifest: delegation.human_in_the_loop.approval_mechanism must be one of [oauth_consent, uma, custom], got '${mech}'`);
        }
    }
    ...
    if (manifest.compliance?.sensitivity) {
        if (!VALID_SENSITIVITY_VALUES.has(manifest.compliance.sensitivity)) {
            errors.push(`manifest: compliance.sensitivity must be one of [confidential, internal, public, restricted], got '${manifest.compliance.sensitivity}'`);
        }
    }
    // Warn if any unit requires auth but no root-level auth block is present (§7)
    const hasProtected = manifest.units.some((u) => u.access === "authenticated" || u.access === "restricted");
    if (hasProtected && (!manifest.auth || !manifest.auth.methods.length)) {
        warnings.push("manifest: units with access 'authenticated' or 'restricted' exist but no 'auth' block is declared");
    }

...
public final class AccessDecisionEngine {
     ...
     * Convention: read:{unit-id-last-segment} or read:{auth_scope} if set.
     */
    private String scopeForUnit(KnowledgeUnit unit) {
        // Use auth_scope if present, otherwise derive from unit id
        if (unit.authScope() != null) {
            // Map auth_scope to a read scope -- the agent card scopes use read: prefix
